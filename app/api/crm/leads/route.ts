...
export async function GET(req:NextRequest){if(!authenticated(req))return json({error:'Inicia sessão para continuar.'},401);return json({leads:listCandidates()});}
export async function POST(req:NextRequest){
 ...
 try{
  ...
  return json({lead:addCandidate(candidate,null,'manual-'+key,digest(JSON.stringify(input)))},201);
